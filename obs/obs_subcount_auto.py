...
def check_for_updates_async():
    ...
    if not UPDATE_MODULE_AVAILABLE:
        log_message("⚠️ Module updater non disponible - vérification ignorée")
        return
    
    if not REQUESTS_AVAILABLE:
        log_message("⚠️ Module requests non disponible - vérification ignorée")
        return
    ...
    try:
        ...
        log_message(f"📦 Version actuelle: {current_ver}")
        
        update_info = check_for_updates()
        
        if update_info is None:
            log_message("⚠️ Impossible de vérifier les mises à jour (pas de connexion ou erreur)")
        elif update_info.get('available'):
            latest = update_info.get('latest_version')
            log_message("")
            log_message("=" * 60)
            log_message(f"🎉 MISE À JOUR DISPONIBLE: v{latest}")
            log_message("=" * 60)
            log_message(f"📌 Version actuelle: v{current_ver}")
            log_message(f"📌 Nouvelle version: v{latest}")
            
            if update_info.get('download_url'):
                log_message(f"📥 Téléchargement: {update_info.get('download_url')}")
            
            if update_info.get('release_notes'):
                notes = update_info.get('release_notes')[:200]  # Limiter la longueur
                log_message(f"📝 Notes: {notes}...")
            
            log_message("")
            log_message("💡 Visitez GitHub pour télécharger la mise à jour")
            log_message("=" * 60)
        else:
            log_message(f"✅ Vous utilisez la dernière version ({current_ver})")
        
        log_message("-" * 60)
        log_message("")
        
    except Exception as e:
        log_message(f"❌ Erreur vérification mises à jour: {e}")
...
def check_dependencies():
    ...
    # 1. Vérifier Python
    log_message("1️⃣ Vérification de Python...")
    try:
        import sys
        python_version = sys.version_info
        version_str = f"{python_version.major}.{python_version.minor}.{python_version.micro}"
        log_message(f"   ✅ Python {version_str} détecté")
        ...
        if python_version.major != 3 or python_version.minor != 6:
            ...
            log_message(f"   ⚠️  Version non optimale pour OBS (recommandé: 3.6.x)")
    except Exception as e:
        errors.append(f"Impossible de vérifier Python: {e}")
        log_message(f"   ❌ Erreur: {e}")
    
    # 2. Vérifier Node.js
    log_message("2️⃣ Vérification de Node.js...")
    try:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            log_message(f"   ✅ npm {npm_version} installé")
        else:
            ...
            if os.path.exists(node_modules_path):
                ...
                log_message(f"   ⚠️  npm non détecté mais node_modules existe")
            else:
                errors.append("npm introuvable ou non fonctionnel")
                log_message(f"   ❌ npm non détecté")
    except FileNotFoundError:
        # npm non trouvé, vérifier si node_modules existe déjà
        node_modules_path = os.path.join(PROJECT_ROOT, 'server', 'node_modules')
        if os.path.exists(node_modules_path):
            warnings.append("npm non détecté dans PATH mais node_modules présent")
            log_message(f"   ⚠️  npm introuvable dans PATH (node_modules existe)")
        else:
            errors.append("npm n'est pas installé ou pas dans PATH")
            log_message(f"   ❌ npm introuvable dans PATH")
    except Exception as e:
        # Erreur npm mais vérifier si node_modules existe
        node_modules_path = os.path.join(PROJECT_ROOT, 'server', 'node_modules')
        if os.path.exists(node_modules_path):
            warnings.append(f"Erreur vérification npm mais node_modules présent: {e}")
            log_message(f"   ⚠️  Erreur npm: {e} (node_modules existe)")
        else:
            errors.append(f"Erreur vérification npm: {e}")
            log_message(f"   ❌ Erreur: {e}")
    
    # 4. Vérifier les modules Python
    log_message("4️⃣ Vérification des modules Python...")
    
    if not PSUTIL_AVAILABLE:
        warnings.append("Module Python 'psutil' manquant - gestion processus limitée")
        log_message(f"   ⚠️  psutil manquant (fonctionnalités limitées)")
    else:
        log_message(f"   ✅ psutil disponible")
    
    if not REQUESTS_AVAILABLE:
        warnings.append("Module Python 'requests' manquant - API désactivée")
        log_message(f"   ⚠️  requests manquant (API désactivée)")
    else:
        log_message(f"   ✅ requests disponible")
    
    try:
        import websocket
        log_message(f"   ✅ websocket-client disponible")
    except ImportError:
        errors.append("Module Python 'websocket-client' manquant")
        log_message(f"   ❌ websocket-client manquant (requis pour OBS)")
    
    # 5. Vérifier les fichiers essentiels
    log_message("5️⃣ Vérification des fichiers...")
    ...
    for file, description in essential_files.items():
        ...
        if os.path.exists(file_path):
            log_message(f"   ✅ {os.path.basename(file)} ({description})")
        else:
            errors.append(f"Fichier manquant: {os.path.basename(file)} ({description})")
            log_message(f"   ❌ {os.path.basename(file)} MANQUANT")
    
    # 6. Vérifier node_modules
    log_message("6️⃣ Vérification des dépendances Node.js...")
    node_modules_path = os.path.join(PROJECT_ROOT, 'server', 'node_modules')
    if os.path.exists(node_modules_path):
        log_message(f"   ✅ Dossier node_modules présent")
        ...
        for module in critical_modules:
            ...
            if os.path.exists(module_path):
                log_message(f"   ✅ {module} installé")
            else:
                warnings.append(f"Module Node.js '{module}' manquant")
                log_message(f"   ⚠️  {module} manquant (npm install requis)")
    else:
        errors.append("Dossier node_modules manquant - exécutez 'npm install'")
        log_message(f"   ❌ node_modules MANQUANT (npm install requis)")
    
    # Résumé
    log_message("=" * 60)
    
    if errors:
        log_message(f"❌ ÉCHEC: {len(errors)} erreur(s) critique(s)")
        for i, error in enumerate(errors, 1):
            log_message(f"   {i}. {error}")
        
        if warnings:
            log_message(f"⚠️  {len(warnings)} avertissement(s):")
            for i, warning in enumerate(warnings, 1):
                log_message(f"   {i}. {warning}")
        
        log_message("")
        log_message("📋 SOLUTION:")
        log_message("   Relancez INSTALLER.bat en mode administrateur")
        log_message("   Ou installez manuellement:")
        log_message("   • Node.js: https://nodejs.org/")
        log_message("   • Python 3.6.8: https://www.python.org/downloads/release/python-368/")
        log_message("   • Modules Python: pip install websocket-client psutil requests")
        log_message("   • Modules Node: npm install")
        log_message("=" * 60)
        
        return False, errors
    
    elif warnings:
        log_message(f"⚠️  ATTENTION: {len(warnings)} avertissement(s)")
        for i, warning in enumerate(warnings, 1):
            log_message(f"   {i}. {warning}")
        log_message("✅ Le serveur peut démarrer avec des fonctionnalités limitées")
        log_message("=" * 60)
        
        return True, warnings
    
    else:
        log_message("✅ TOUTES LES DÉPENDANCES SONT INSTALLÉES")
        log_message("=" * 60)
...
def find_subcount_processes():
    ...
    try:
        ...
        log_message(f"Erreur lors de la recherche des processus: {e}")
...
def kill_existing_servers():
    ...
    if processes:
        log_message(f"🔄 Arrêt de {len(processes)} processus SubCount Auto existants...")
        for proc in processes:
            try:
                log_message(f"   ⏹️ Arrêt du processus {proc.pid} ({proc.info['name']})")
                proc.terminate()
                # Attendre 3 secondes pour un arrêt propre
                proc.wait(timeout=3)
            except psutil.TimeoutExpired:
                log_message(f"   💥 Force l'arrêt du processus {proc.pid}")
                proc.kill()
            except Exception as e:
                log_message(f"   ❌ Erreur arrêt processus {proc.pid}: {e}")
        
        # Vérifier que tout est bien arrêté
        time.sleep(1)
        remaining = find_subcount_processes()
        if remaining:
            log_message(f"⚠️ {len(remaining)} processus toujours actifs")
        else:
            log_message("✅ Tous les processus SubCount Auto arrêtés")
...
def start_server():
    ...
    try:
        log_message("")
        log_message("=" * 60)
        log_message("🚀 DÉMARRAGE DU SERVEUR SUBCOUNT AUTO")
        log_message("=" * 60)
        log_message("")
        ...
        if not deps_ok:
            log_message("")
            log_message("🛑 SERVEUR NON DÉMARRÉ - Dépendances manquantes")
            log_message("   Consultez les erreurs ci-dessus et lancez INSTALLER.bat")
            log_message("")
            return False
        ...
        if deps_issues:
            log_message("")
            log_message("⚠️  Démarrage avec des fonctionnalités limitées...")
            log_message("")
        
        # Vérifier que le fichier START_SERVER.bat existe
        if not os.path.exists(START_SERVER_BAT):
            log_message(f"❌ Fichier START_SERVER.bat introuvable: {START_SERVER_BAT}")
            return False
        
        log_message("� Arrêt des serveurs existants...")
        ...
        log_message(f"✅ Serveur SubCount Auto démarré (PID: {server_process.pid})")
        
        # Attendre un peu pour vérifier que le serveur démarre bien
        time.sleep(3)
        
        if server_process.poll() is None:
            log_message("✅ Serveur SubCount Auto en cours d'exécution")
            return True
        else:
            log_message("❌ Le serveur SubCount Auto s'est arrêté immédiatement")
            is_server_running = False
            return False
            
    except Exception as e:
        log_message(f"❌ Erreur démarrage serveur: {e}")
        is_server_running = False
        return False
...
def stop_server():
    ...
    log_message("🔄 Arrêt du serveur SubCount Auto...")
    
    # Arrêter le processus principal si il existe
    if server_process:
        try:
            log_message(f"   ⏹️ Arrêt du processus principal {server_process.pid}")
            server_process.terminate()
            server_process.wait(timeout=5)
        except subprocess.TimeoutExpired:
            log_message("   💥 Force l'arrêt du processus principal")
            server_process.kill()
        except Exception as e:
            log_message(f"   ❌ Erreur arrêt processus principal: {e}")
    ...
    server_process = None
    is_server_running = False
    log_message("✅ Serveur SubCount Auto arrêté")
    ...
    while is_server_running:
        try:
            if server_process and server_process.poll() is not None:
                log_message("⚠️ Le serveur SubCount Auto s'est arrêté de manière inattendue")
                is_server_running = False
                break
            ...
            log_message(f"❌ Erreur monitoring serveur: {e}")
            break
...
def add_follow():
    ...
    try:
        ...
        if response.status_code == 200:
            log_message("✅ +1 Follow ajouté")
            return True
    except Exception as e:
        log_message(f"❌ Erreur ajout follow: {e}")
    return False
...
def remove_follow():
    ...
    try:
        ...
        if response.status_code == 200:
            log_message("✅ -1 Follow retiré")
            return True
    except Exception as e:
        log_message(f"❌ Erreur retrait follow: {e}")
    return False
...
def script_load(settings):
    ...
    log_message("🎬 Script OBS SubCount Auto v2.1 avec Auto-Update chargé")
    log_message(f"📂 Répertoire: {SCRIPT_DIR}")
    log_message(f"🚀 Fichier serveur: {START_SERVER_BAT}")
...
def script_unload():
    ...
    log_message("🎬 Script OBS SubCount Auto déchargé")
    is_server_running = False
    
    # Arrêter le serveur
    stop_server()
    
    log_message("👋 Arrêt complet du script OBS SubCount Auto")
...
def restart_server():
    ...
    log_message("🔄 Redémarrage manuel du serveur...")
    stop_server()
    time.sleep(2)
